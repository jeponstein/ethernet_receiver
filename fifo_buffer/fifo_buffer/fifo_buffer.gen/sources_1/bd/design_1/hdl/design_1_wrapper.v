//Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
//Copyright 2022-2024 Advanced Micro Devices, Inc. All Rights Reserved.
//--------------------------------------------------------------------------------
//Tool Version: Vivado v.2024.2 (win64) Build 5239630 Fri Nov 08 22:35:27 MST 2024
//Date        : Wed Apr  2 11:22:31 2025
//Host        : S20223108 running 64-bit major release  (build 9200)
//Command     : generate_target design_1_wrapper.bd
//Design      : design_1_wrapper
//Purpose     : IP block netlist
//--------------------------------------------------------------------------------
`timescale 1 ps / 1 ps

module design_1_wrapper
   (DDR_addr,
    DDR_ba,
    DDR_cas_n,
    DDR_ck_n,
    DDR_ck_p,
    DDR_cke,
    DDR_cs_n,
    DDR_dm,
    DDR_dq,
    DDR_dqs_n,
    DDR_dqs_p,
    DDR_odt,
    DDR_ras_n,
    DDR_reset_n,
    DDR_we_n,
    FIXED_IO_ddr_vrn,
    FIXED_IO_ddr_vrp,
    FIXED_IO_mio,
    FIXED_IO_ps_clk,
    FIXED_IO_ps_porb,
    FIXED_IO_ps_srstb,
    gmii_rtl_gtx_clk,
    gmii_rtl_rx_clk,
    gmii_rtl_rx_dv,
    gmii_rtl_rx_er,
    gmii_rtl_rxd,
    gmii_rtl_tx_clk,
    gmii_rtl_tx_en,
    gmii_rtl_tx_er,
    gmii_rtl_txd,
    mdio_rtl_mdc,
    mdio_rtl_mdio_io,
    reset_rtl,
    sys_clock);
  inout [14:0]DDR_addr;
  inout [2:0]DDR_ba;
  inout DDR_cas_n;
  inout DDR_ck_n;
  inout DDR_ck_p;
  inout DDR_cke;
  inout DDR_cs_n;
  inout [3:0]DDR_dm;
  inout [31:0]DDR_dq;
  inout [3:0]DDR_dqs_n;
  inout [3:0]DDR_dqs_p;
  inout DDR_odt;
  inout DDR_ras_n;
  inout DDR_reset_n;
  inout DDR_we_n;
  inout FIXED_IO_ddr_vrn;
  inout FIXED_IO_ddr_vrp;
  inout [53:0]FIXED_IO_mio;
  inout FIXED_IO_ps_clk;
  inout FIXED_IO_ps_porb;
  inout FIXED_IO_ps_srstb;
  output gmii_rtl_gtx_clk;
  input gmii_rtl_rx_clk;
  input gmii_rtl_rx_dv;
  input gmii_rtl_rx_er;
  input [7:0]gmii_rtl_rxd;
  input gmii_rtl_tx_clk;
  output gmii_rtl_tx_en;
  output gmii_rtl_tx_er;
  output [7:0]gmii_rtl_txd;
  output mdio_rtl_mdc;
  inout mdio_rtl_mdio_io;
  output [0:0]reset_rtl;
  input sys_clock;

  wire [14:0]DDR_addr;
  wire [2:0]DDR_ba;
  wire DDR_cas_n;
  wire DDR_ck_n;
  wire DDR_ck_p;
  wire DDR_cke;
  wire DDR_cs_n;
  wire [3:0]DDR_dm;
  wire [31:0]DDR_dq;
  wire [3:0]DDR_dqs_n;
  wire [3:0]DDR_dqs_p;
  wire DDR_odt;
  wire DDR_ras_n;
  wire DDR_reset_n;
  wire DDR_we_n;
  wire FIXED_IO_ddr_vrn;
  wire FIXED_IO_ddr_vrp;
  wire [53:0]FIXED_IO_mio;
  wire FIXED_IO_ps_clk;
  wire FIXED_IO_ps_porb;
  wire FIXED_IO_ps_srstb;
  wire gmii_rtl_gtx_clk;
  wire gmii_rtl_rx_clk;
  wire gmii_rtl_rx_dv;
  wire gmii_rtl_rx_er;
  wire [7:0]gmii_rtl_rxd;
  wire gmii_rtl_tx_clk;
  wire gmii_rtl_tx_en;
  wire gmii_rtl_tx_er;
  wire [7:0]gmii_rtl_txd;
  wire mdio_rtl_mdc;
  wire mdio_rtl_mdio_i;
  wire mdio_rtl_mdio_io;
  wire mdio_rtl_mdio_o;
  wire mdio_rtl_mdio_t;
  wire [0:0]reset_rtl;
  wire sys_clock;

  design_1 design_1_i
       (.DDR_addr(DDR_addr),
        .DDR_ba(DDR_ba),
        .DDR_cas_n(DDR_cas_n),
        .DDR_ck_n(DDR_ck_n),
        .DDR_ck_p(DDR_ck_p),
        .DDR_cke(DDR_cke),
        .DDR_cs_n(DDR_cs_n),
        .DDR_dm(DDR_dm),
        .DDR_dq(DDR_dq),
        .DDR_dqs_n(DDR_dqs_n),
        .DDR_dqs_p(DDR_dqs_p),
        .DDR_odt(DDR_odt),
        .DDR_ras_n(DDR_ras_n),
        .DDR_reset_n(DDR_reset_n),
        .DDR_we_n(DDR_we_n),
        .FIXED_IO_ddr_vrn(FIXED_IO_ddr_vrn),
        .FIXED_IO_ddr_vrp(FIXED_IO_ddr_vrp),
        .FIXED_IO_mio(FIXED_IO_mio),
        .FIXED_IO_ps_clk(FIXED_IO_ps_clk),
        .FIXED_IO_ps_porb(FIXED_IO_ps_porb),
        .FIXED_IO_ps_srstb(FIXED_IO_ps_srstb),
        .gmii_rtl_gtx_clk(gmii_rtl_gtx_clk),
        .gmii_rtl_rx_clk(gmii_rtl_rx_clk),
        .gmii_rtl_rx_dv(gmii_rtl_rx_dv),
        .gmii_rtl_rx_er(gmii_rtl_rx_er),
        .gmii_rtl_rxd(gmii_rtl_rxd),
        .gmii_rtl_tx_clk(gmii_rtl_tx_clk),
        .gmii_rtl_tx_en(gmii_rtl_tx_en),
        .gmii_rtl_tx_er(gmii_rtl_tx_er),
        .gmii_rtl_txd(gmii_rtl_txd),
        .mdio_rtl_mdc(mdio_rtl_mdc),
        .mdio_rtl_mdio_i(mdio_rtl_mdio_i),
        .mdio_rtl_mdio_o(mdio_rtl_mdio_o),
        .mdio_rtl_mdio_t(mdio_rtl_mdio_t),
        .reset_rtl(reset_rtl),
        .sys_clock(sys_clock));
  IOBUF mdio_rtl_mdio_iobuf
       (.I(mdio_rtl_mdio_o),
        .IO(mdio_rtl_mdio_io),
        .O(mdio_rtl_mdio_i),
        .T(mdio_rtl_mdio_t));
endmodule
